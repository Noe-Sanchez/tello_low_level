...
struct tello tello;
struct bar {GtkWidget *box; GtkWidget *label; GtkWidget *bar;};
struct bar bars[8];
GtkWidget *buttons[8];
GtkWidget *infolabel;
pid_t pid1 = 0;
...
void button_callback(GtkWidget *widget, void *ptr)
{
	...
	case 0:
		if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)) == FALSE) {
			...
			pid1 = fork();
			if(pid1 == 0) {
				execlp("ffplay", "ffplay", "-fflags", "nobuffer", "-flags", "low_delay", "-framedrop", "-sync", "ext", "-probesize", "32", "-framerate", "60", "udp://0.0.0.0:11111", NULL);
				exit(EXIT_SUCCESS);
			}
		}
		return;
	...
	switch (id) {
		case 26: {
			if(wifi_strength != tello.wifi_strength) {
				...
				sprintf(label, "Wifi\n%d%%", wifi_strength);
				gtk_label_set_text(GTK_LABEL(bars[1].label), label);
				gtk_level_bar_set_value(GTK_LEVEL_BAR(bars[1].bar), wifi_strength);
			}
			break;
		}
		case 86: {
			...
			if(battery_percentage != tello.battery_percentage) {
				...
				sprintf(label, "Battery\n%d%%", battery_percentage);
				gtk_label_set_text(GTK_LABEL(bars[0].label), label);
				gtk_level_bar_set_value(GTK_LEVEL_BAR(bars[0].bar), battery_percentage);
			}
			if(height != tello.height) {
				height = tello.height;
				sprintf(label, "Height\n%.1fm", height/10.0);
				gtk_label_set_text(GTK_LABEL(bars[2].label), label);
				gtk_level_bar_set_value(GTK_LEVEL_BAR(bars[2].bar), height < 0 ? 0 : height);
			}
...
void on_activate(GtkApplication *app)
{
	...
	gtk_grid_attach(GTK_GRID(grid), actionbar, 0, 1, 1, 1);

	create_button("camera", actionbar, 1);
	create_button("media-floppy", actionbar, 1);
	create_button("display", actionbar, 1);
	create_button("speedometer", actionbar, 1);
	create_button("up", actionbar, 1);
	create_button("joystick", actionbar, 1);
...
{
	GtkApplication *app = gtk_application_new("com.tello.GtkApplication", G_APPLICATION_FLAGS_NONE);
